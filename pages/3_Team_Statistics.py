...
# New Section: Match Comments and Q/A
st.subheader("Match Comments and Q/A")

# Define the columns to display (updated to match form_config.py)
qa_columns = ['match_number', 'alliance_color', 'match_outcome_final', 'defense_qa', 'teleop_qa', 'auto_qa', 'comments']

# Check if all required columns exist in team_data
...
if missing_cols:
    ...
    # Sort by match number in descending order (highest to lowest)
    display_df = display_df.sort_values('Match Number', ascending=False)

    # Standardize "no comment" variations to "No Information Provided"
    # First, fill NaN values
    display_df = display_df.fillna("No Information Provided")
    # Replace empty strings and variations of "no comment"
    ...
    # Apply custom CSS to control column widths and left-align the Match Number column
    st.markdown("""
        <style>
        /* Target the table generated by st.dataframe */
        .stDataFrame {
            width: 100%;
        }
        /* Style the table to control column widths */
        .stDataFrame table {
            table-layout: fixed;
            width: 100%;
        }
        /* Define column widths */
        .stDataFrame th:nth-child(1), .stDataFrame td:nth-child(1) {  /* Match Number */
            width: 10%;
            text-align: left !important;  /* Left-align Match Number */
        }
        .stDataFrame th:nth-child(2), .stDataFrame td:nth-child(2) {  /* Alliance Color */
            width: 10%;
        }
        .stDataFrame th:nth-child(3), .stDataFrame td:nth-child(3) {  /* Match Outcome */
            width: 10%;
        }
        .stDataFrame th:nth-child(4), .stDataFrame td:nth-child(4) {  /* Defense Q/A */
            width: 17.5%;
        }
        .stDataFrame th:nth-child(5), .stDataFrame td:nth-child(5) {  /* Teleop Q/A */
            width: 17.5%;
        }
        .stDataFrame th:nth-child(6), .stDataFrame td:nth-child(6) {  /* Autonomous Q/A */
            width: 17.5%;
        }
        .stDataFrame th:nth-child(7), .stDataFrame td:nth-child(7) {  /* Additional Comments */
            width: 17.5%;
        }
        /* Ensure text wraps within the columns */
        .stDataFrame td {
