...
if pit_df is None or pit_df.empty:
    st.warning("No pit scouting data available. Pit scouting statistics will be unavailable.")
    pit_df = pd.DataFrame()
...
# Team selection
if 'team_number' in match_df.columns:
    team_numbers = sorted(match_df['team_number'].unique())
    selected_team = st.selectbox("Select a Team", options=team_numbers)
else:
    st.error("Team number column not found in match data.")
    st.stop()

# Filter data for the selected team
team_data = match_df[match_df['team_number'] == selected_team]
team_pit_data = pit_df[pit_df['team_number'] == selected_team] if not pit_df.empty else pd.DataFrame()

if team_data.empty:
    st.info(f"No match data available for team {selected_team}.")
    st.stop()
